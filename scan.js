...
// ── Delete HR Contacts ────────────────────────────────────────────────────────
async function deleteHrContacts(req, res) {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/contacts?source=like.Herold*&source=like.CEE*&source=like.Masterlist*`, {
    method: 'DELETE',
    headers: { 'apikey': SUPABASE_SERVICE_KEY, 'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`, 'Content-Type': 'application/json' }
  });
...
async function importHrContacts(req, res) {
  ...
  // Alle auf einmal als Batch einfügen
